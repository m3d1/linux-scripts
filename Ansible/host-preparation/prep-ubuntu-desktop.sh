...
### ---------------------------
### 5) Create its (sudo) users: its-sd, its-admin
### ---------------------------
log "Creating its users (non-sudo): ${ITS_USERS[*]} ..."
for u in "${ITS_USERS[@]}"; do
  create_its_user "$u"
done

##hardcoded passwords
sudo chpasswd -e <<EOF
its-sd:\$y\$j9T\$efigzrwV7ykf6ZRX6Dl0n1\$Qkgi6.eu9AKKXcq9toBYsPuj7zqv4g1zNNuuNb/YhY5
